...
{
public:
    Integrator(Bounds2i bounds, Scene* s, std::shared_ptr<Sampler> _sampler, int recursionLimit)
        : scene(s), camera(s->camera), film(&(s->film)), sampler(_sampler),
          bounds(bounds), recursionLimit(recursionLimit)
    {
        camera->sampler = _sampler;
        ClampBounds();
    }
...
protected:
    ...
    std::shared_ptr<Camera>  camera;			// A pointer to the Camera instance stored in MyGL.
