...
void Film::WriteImage(QString path)
{
    ...
    for(unsigned int i = 0; i < dims.x; i++)
    {
        for(unsigned int j = 0; j < dims.y; j++)
        {
            ...
            //color = glm::clamp(color, Color3f(0.f,0.f,0.f), Color3f(255.f,255.f,255.f));
            glm::ivec3 color = (glm::ivec3)((pixels[i][j]) * 255.f);
            color = glm::clamp(color, glm::ivec3(0,0,0),glm::ivec3(255,255,255));
