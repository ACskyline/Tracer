...
void MyGL::GLDrawScene()
{
    ...
    prog_flat.draw(*this, *scene.camera.get());
}
...
void MyGL::keyPressEvent(QKeyEvent *e)
{
    ...
    if (e->key() == Qt::Key_Escape) {
        ...
        scene.camera = std::make_shared<Camera>(gl_camera);
...
void MyGL::RenderScene()
{
    ...
    if(integratorType == PHOTON_MAPPING)
    {
        ...
        std::vector<std::vector<Photon>>* causticSets = new std::vector<std::vector<Photon>>(8);
        for(int i = 0; i < 8; ++i)
        {
            ...
            int seed = i;
            PhotonMapper* pm = new PhotonMapper(threadPhotonCount, &((*photonSets)[i]), &((*causticSets)[i]), &scene, sampler->Clone(seed), recursionLimit);
            QThreadPool::globalInstance()->start(pm);
        }
        QThreadPool::globalInstance()->waitForDone();//new
        std::vector<Photon>* allPhotons = new std::vector<Photon>();
        std::vector<Photon>* allCaustics = new std::vector<Photon>();
        ...
        for(std::vector<Photon>& ps : *causticSets)
        {
            allCaustics->insert(allCaustics->end(), ps.begin(), ps.end());
        }
        delete photonSets;
        delete causticSets;
        ...
        kdTreeCaustic.build(allCaustics);
    ...
    for(int X = 0; X < nTiles.x; X++)
    {
        for(int Y = 0; Y < nTiles.y; Y++)
        {
            ...
            case NAIVE_LIGHTING:
                ...
                break;
            case PHOTON_MAPPING:
                rt = new PhotonMapper(tileBounds, &scene, sampler->Clone(seed), recursionLimit, &kdTree, &kdTreeCaustic, photonSearchRadius);
                break;
            }
#ifndef MY_GLOBAL_DEBUG
...
{
    std::cout << "Milliseconds to render: " << ((long)QDateTime::currentMSecsSinceEpoch() - renderTime) << std::endl;
    is_rendering = false;
    something_rendered = true;
