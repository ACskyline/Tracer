...
SquarePlane::SquarePlane(float _side, Transform _transform) : Shape(_transform), side(_side)
{

}

bool SquarePlane::getIntersection(Ray r, Intersection *i)
{
    ...
    else
    {
        ...
        if(pLocal.x<-side/2.f||pLocal.x>side/2.f||pLocal.y<-side/2.f||pLocal.y>side/2.f)//assuming square in x-y plane and axis aligned
        {
            return false;
        }
        else
